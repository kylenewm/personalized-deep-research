...
##########################
# S05: Verified Findings Selector Mode
##########################

verified_findings_selector_prompt = """
## VERIFIED FINDINGS SECTION (SELECTOR MODE)

You MUST include a "Verified Findings" section in your report. This section contains ONLY quotes that have been verified to exist in source documents.

<AVAILABLE_VERIFIED_QUOTES>
{verified_quotes}
</AVAILABLE_VERIFIED_QUOTES>

SELECTOR MODE RULES (CRITICAL - NO EXCEPTIONS):
1. Select 3-5 of the MOST RELEVANT quotes from the AVAILABLE_VERIFIED_QUOTES list above
2. Copy each quote EXACTLY as written - do NOT paraphrase, modify, or summarize
3. Format each quote as: * "[Exact Quote]" - [Source Title](URL)
4. If fewer than 3 quotes are available, include all of them
5. If NO quotes are available, write: "No verified quotes available for this research."

NEVER:
- Invent or fabricate quotes
- Modify the wording of quotes
- Use quotes not in the AVAILABLE_VERIFIED_QUOTES list
- Paraphrase the quotes in any way

The Verified Findings section proves your report is grounded in real source material.
"""

verified_findings_disabled_message = """
## Verified Findings

*Verification was disabled for this research run. The Trust Store was unavailable.*
